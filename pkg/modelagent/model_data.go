...
func StorageIdentityForSpec(spec *v1beta1.BaseModelSpec) (storageURI string, storagePath string, ok bool) {
	...
	return StorageIdentityForStorageSpec(spec.Storage)
}

// StorageIdentityForStorageSpec returns the storage fields that identify the model artifact on disk.
func StorageIdentityForStorageSpec(storageSpec *v1beta1.StorageSpec) (storageURI string, storagePath string, ok bool) {
	if storageSpec == nil {
		return "", "", false
	}
	if storageSpec.StorageUri != nil {
		storageURI = *storageSpec.StorageUri
	}
	if storageSpec.Path != nil {
		storagePath = *storageSpec.Path
	}
