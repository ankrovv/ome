...
func (c *ConfigMapReconciler) ReconcileModelStatus(ctx context.Context, statusOp *ConfigMapStatusOp) error {
	...
	_, _, hasCurrentStorageIdentity := StorageIdentityForSpec(spec)
	...
	if !exists {
		...
		if !cacheEntry.matchesStorageIdentity(spec) ||
			(hasCurrentStorageIdentity && statusOp.ModelStatus == ModelStatusUpdating && !cacheEntry.hasStorageIdentity()) {
...
func (c *ConfigMapReconciler) updateModelProgressInConfigMap(ctx context.Context, configMap *corev1.ConfigMap, op *ConfigMapProgressOp, needCreate bool) error {
	...
	spec := getModelSpec(op.BaseModel, op.ClusterBaseModel)
	_, _, hasCurrentStorageIdentity := StorageIdentityForSpec(spec)
	if !modelEntry.MatchesStorageIdentity(spec) ||
		(hasCurrentStorageIdentity && op.Progress != nil && !modelEntry.hasStorageIdentity()) {
	...
	modelEntry.ApplyStorageIdentity(spec)
...
func (c *ConfigMapReconciler) updateModelStatusInConfigMap(ctx context.Context, configMap *corev1.ConfigMap, op *ConfigMapStatusOp, needCreate bool) error {
	...
	if existingData, exists := configMap.Data[key]; exists {
		...
		if err := json.Unmarshal([]byte(existingData), &modelEntry); err != nil {
			...
			_, _, hasCurrentStorageIdentity := StorageIdentityForSpec(spec)
			if !modelEntry.MatchesStorageIdentity(spec) ||
				(hasCurrentStorageIdentity && op.ModelStatus == ModelStatusUpdating && !modelEntry.hasStorageIdentity()) {
				modelEntry.Config = nil
